...
<template>
	<div>
		<mavon-editor v-model="value" ref="md" @imgAdd="imgAdd" @change="change" @save="save" @imgDel = "imgDel" style="min-height: 600px" />
	</div>
</template>
<script>
	import {
	...
	export default {
		...
		methods: {
			 ...
			 */
			imgDel(imgName){
				window.console.log("===",imgName[0],"===");
				let name = imgName[0];
				name = name.substr(name.lastIndexOf("/") + 1);
				this.$emit('imgDel',name);
			},
			/**
			 * 获取文本内容
			 */
